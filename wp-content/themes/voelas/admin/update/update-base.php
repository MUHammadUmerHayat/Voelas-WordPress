...
if(!class_exists("DFWPThemeActivationBase")) {
	class DFWPThemeActivationBase {
		public $key = "2BFEE74C56665587";
		private $product_id = "5";
		private $product_base = "voelas";
		private $server_host = "https://df-licensing.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_voelas_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=voelas_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		{
			$response = $this->plugin_updateInfo();
		...
		private function processs_response($response){
			...
			if ( ! empty( $response ) ) {
				...
				if ( is_object( $response ) ) {
					...
					$response->msg    = "Response Error, contact with the author or update the theme";
					if(!empty($bkjson)){
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
					...
					if ( ! empty( $response->status ) ) {
						if ( ! empty( $response->data ) ) {
							...
							if ( $licenseObj->is_valid ) {
								...
								if ( $this->checkoldtied( $oldRespons, $responseObj, $response ) ) {
									return true;
								} else {
									$this->removeOldWPResponse();
			...
			return $this->checkoldtied($oldRespons,$responseObj);
		}
		private function checkoldtied(&$oldRespons,&$responseObj){
