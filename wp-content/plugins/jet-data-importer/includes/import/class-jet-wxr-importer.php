...
class Jet_WXR_Importer extends WP_Importer {
	...
	public function process_single_table( $table, $data ) {
		...
		if( in_array( $table, $parsed_array ) ){
			/*$data = preg_replace_callback(
				'/:["\']([^"\']*)(\\\\\/[0-9]{4}\\\\\/[0-9]{2}\\\\\/[^"\']*(jpg|jpeg|png|gif|svg))["\']/',
				function ( $matches ) {
					$upload_dir = wp_upload_dir();
					$upload_dir = json_encode( $upload_dir['baseurl'] );
					$upload_dir = str_replace( '"', '', $upload_dir );

					$new_url = str_replace( $matches[1], $upload_dir, $matches[0] );
			...
			if( 'revslider_navigations' === $table ){
				$data = preg_replace_callback(
					'/\s{2,}/',
					function ( $matches ) {
						return '';
